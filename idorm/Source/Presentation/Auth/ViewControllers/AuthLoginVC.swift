...
final class AuthLoginViewController: BaseViewController, View {
  
  typealias Reactor = AuthLoginViewReactor
  ...
  private let emailTextField: iDormTextField = {
    let textField = iDormTextField(type: .login)
  ...
  private let passwordTextField: iDormTextField = {
    let textField = iDormTextField(type: .login)
  ...
  func bind(reactor: AuthLoginViewReactor) {
    ...
    reactor.pulse(\.$shouldNavigateToEmailVC)
      ...
      .drive(with: self) { owner, authProcess in
        let viewController = AuthEmailViewController()
        switch authProcess {
        case .findPw:
          viewController.reactor = AuthEmailViewReactor(.findPassword)
        case .signUp:
          viewController.reactor = AuthEmailViewReactor(.signUp)
        }
  ...
  override func setupConstraints() {
    ...
    self.forgotPasswordButton.snp.makeConstraints { make in
      ...
      make.top.equalTo(self.loginButton.snp.bottom).offset(10.0)
    }
