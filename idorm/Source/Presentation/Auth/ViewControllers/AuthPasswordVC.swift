...
final class AuthPasswordViewController: BaseViewController, View {
  
  typealias Reactor = AuthPasswordViewReactor
  ...
  private let verifyPasswordTextField: iDormTextField = {
    let textField = iDormTextField(type: .withBorderLine)
  ...
  func bind(reactor: AuthPasswordViewReactor) {
    ...
    reactor.pulse(\.$navigateToNicknameVC).skip(1)
      ...
      .drive(with: self) { owner, _ in
        ...
        viewController.reactor = AuthNicknameViewReactor(.signUp)
