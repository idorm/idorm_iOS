...
final class DormCalendarCell: UICollectionViewCell, BaseViewProtocol {
  ...
  func configure(with calendar: DormCalendar) {
    ...
    // Date
    self.dateLabel.text = calendar.startDate
    if let endDate = calendar.endDate {
      self.dateLabel.text?.append(" ~ \(endDate)")
    }
    ...
    if calendar.startTime == nil,
      ...
      self.scheduleTimeLabel.text = calendar.startTime
      if let endTime = calendar.endTime {
        self.scheduleTimeLabel.text?.append(" ~ \(endTime)")
      }
    }
